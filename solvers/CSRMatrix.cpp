...
std::vector<double> CSRMatrix::operator*(const std::vector<double> &x) const {
    if (x.size() != n) {
...
std::vector<double> CSRMatrix::get_diag() const {
    if (n != m) {
        throw std::runtime_error("Can't get diagonal of not square matrix");
    }
